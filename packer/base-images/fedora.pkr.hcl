...
variable "base_image_ocid" {
  type        = string
  description = "OCID of the base Oracle Linux image (or Fedora if available in marketplace)"
}

# Source: OCI Compute Instance
source "oracle-oci" "fedora-base" {
  ...
  # Use flexible shape for cost efficiency
  ...
  # Image naming
  image_name = "fedora-base-${var.img_sfx}"

  # SSH configuration
  ssh_username = "opc"

  # Instance configuration
  instance_name = "packer-fedora-base-${var.img_sfx}"
}

# Build configuration
build {
  sources = ["source.oracle-oci.fedora-base"]
  ...
  provisioner "shell" {
    inline = [
      "set -e",
      "chmod +x /var/tmp/automation_images/packer/base-images/scripts/fedora-base-setup.sh",
      "sudo /bin/bash /var/tmp/automation_images/packer/base-images/scripts/fedora-base-setup.sh"
    ]
    environment_vars = [
